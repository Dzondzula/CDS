...
class MemberInfoSwiftUI: UIView {
    
    init( data: String) {
        super.init(frame: .zero)
        
        let vc = UIHostingController(rootView: DetailsSectionViews<MemberInfoProperties>(data: data))
        addSubview(vc.view)
        vc.view.translatesAutoresizingMaskIntoConstraints = false
        ...
        vc.view.leftAnchor.constraint(equalTo: leftAnchor).isActive = true
        vc.view.rightAnchor.constraint(equalTo: rightAnchor).isActive = true
    }
